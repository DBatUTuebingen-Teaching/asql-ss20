...
-- Form pairs of rows t1, t2 whose values in column a diffey by at most 1:
...
-- Use EXPLAIN VERBOSE to see the iterated evaluation of the
-- subquery:
-- ┌───────────────────────────────────────────────────────────────────────────────────┐
-- │                                    QUERY PLAN                                     │
-- ├───────────────────────────────────────────────────────────────────────────────────┤
-- │ Seq Scan on public.t t1  (cost=0.00..9665.35 rows=1174 width=41)                  │
-- │   Output: t1.a, t1.b, t1.c, t1.d                                                  │
-- │   Filter: (t1.b <> (SubPlan 1))                                                   │
-- │   SubPlan 1                                                                       │
-- │     ->  Index Scan using t_pkey on public.t t2  (cost=0.15..8.17 rows=1 width=32) │
-- │           Output: t2.b                                                            │
-- │           Index Cond: (t1.a = t2.a)                                               │
-- └───────────────────────────────────────────────────────────────────────────────────┘
...
--   ⚠ t.a is *not* constant in each group (but t.a % 2 is)
--         ↓
